...
class RecommendRequest(BaseModel):
    ...
    bandwidth_mbps: int = Field(100, ge=1)
    ...
    if not os.path.exists(LEAD_TRACKER_FILE):
        with open(LEAD_TRACKER_FILE, "w", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow(
                [
                    "timestamp", "client_name", "nam_name", "industry",
                    "budget_band", "size", "products_already_sold",
                    "recommended_products", "combined_pitch_generated"
                ]
            )


def log_lead(req, budget_band, recommended_products, combined_generated):
    ...
    row = [
        ...
        budget_band,
        req.size or "",
        ",".join(req.products_already_sold),
...
def catalog():
    return {
        "products": [{"id": pid, "name": p.name} for pid, p in PRODUCTS.items()],
        "industries": INDUSTRIES,
        "product_ids": [{"id": pid, "name": PRODUCTS[pid].name} for pid in ALL_PRODUCT_IDS],
...
def api_recommend(req: RecommendRequest):
    result = recommend_products(
        req.industry, req.budget_band, req.bandwidth_mbps,
        req.size, req.products_already_sold
    )
    if not result["recommended"]:
        raise HTTPException(400, "No suitable products found.")
...
def generate(req: GenerateRequest):
    ...
    result = recommend_products(
        req.industry, req.budget_band,
        req.bandwidth_mbps, req.size,
    ...
    product_paths = [os.path.join(PRODUCT_FOLDER, r["pdf"]) for r in recs]
    ...
    log_lead(req, result["budget_band"], recs, True)
...
def download_product_pitch(product_id: str):
    ...
    if not os.path.exists(pdf_path):
        raise HTTPException(404, "Product PDF not found")
